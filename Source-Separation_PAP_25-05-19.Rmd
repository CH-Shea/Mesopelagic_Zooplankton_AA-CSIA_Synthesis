---
title: "Organic Matter Source Separation and Sample Selection - North Atlantic"
author: "Connor Shea"
...
tracers_select <- c("d15NLys","d15NPhe","d15NThr","d15NSer")#,"d15NAla")
SDtracers_select <- c("Sd15NLys","SDd15NPhe","SDd15NThr","SDd15NSer")#,"SDd15NAla")
...
Data.all <- read_excel("Data/old_format/AA-CSIA_PAP.xlsx",
...
ggplot()+
  geom_point(data = Data.sources,
             aes(x=d15NPhe, y=d15NPro, color=Group),
             size = 3)+
  geom_point(data = Data.zoops,
             aes(x=d15NPhe, y=d15NPro, shape=Size),
             size = 2)+
  labs(shape="Zooplankton",
       color="Particles")+
...
This suggests that submicron particles are not relevant to organic matter supply and can thus be excluded from our analysis.
...
Given that surface zooplankton all have $\dNPhe$ values below 0, we can filter out all particle samples with $\dNPhe$ > 0.

Let's try and look at trophic position as well. Because we have so many surface particle samples at PAP, and we really want to look at the inputs of fresh phytodetritus.

```{r source-filtering-surface-TP, dpi=300, fig.width=5, fig.asp=0.8}

ggplot()+
  geom_point(data = subset(Data.sources, 
                           Group == "Surface"),
             aes(x=d15NPhe, y=((d15NAla-d15NPhe - 3.2)/6.2+1), color=Size),
             size = 3)+
  geom_point(data = subset(Data.zoops, 
                           (Depth < 100 & Event == "Day") |
                           (Type == "Metridia" | Type == "Neocalanus")),
             aes(x=d15NPhe, y=((d15NAla-d15NPhe - 3.2)/6.2+1), shape=interaction(Size,Type)),
             size = 2)+
  labs(shape="Zooplankton",
       color="Particles")+
...
Now we will do a series of statistical tests on our data. We will look at four specific tracer sets:

  1. All tracer $\dNAA$ values
  2. Just $\dN$
  3. Just SAA $\dNAA$ values (Phe, Lys, Ser, Gly)
  4. A select group of $\dNAA$ values (Phe, Lys, Thr)
  5. Select AA $\dN$ values with 4 EAA $\dC$ values
...
var.exp.PCA <- 
  data.frame("AAs" = 
               factor(c("SAAs","Select d15N","All d15N + d13C"),
                      levels = c("SAAs","Select d15N","All d15N + d13C")))
...
```{r pres-fig-2, dpi=300, fig.height=4, fig.asp=0.8}

AAs <- c(
  "d15NPhe","d15NLys","d15NThr","d15NAla","d15NGlx"
)

# Fitting PCA and adding Type as a supplimental qualitative variable
PCA.parts = PCA(Data.sources[c(AAs)], 
                scale.unit = TRUE, graph = FALSE)
PCA.zoops = predict.PCA(PCA.parts, 
                        subset(Data.zoops, Event == "Night")[c(AAs)])
...
# Plot PC1&2 results and project variable vectors and print
fviz_pca_biplot(PCA.parts, axes = c(1,2), 
                geom = "point",
                col.ind = Data.sources$Group, 
                addlabels=FALSE, addEllipses = TRUE, 
                col.var = 'grey50', repel=TRUE,
                invisible = "var")+
  labs(color = "Filtered Particles", fill = "Filtered Particles",  shape = "Filtered Particles")+
  xlab("PC1") + ylab("PC2") +
  theme_light() +
...
fviz_pca_biplot(PCA.parts, axes = c(1,2), 
                ...
                col.var = 'grey50', repel=TRUE,
                invisible = "var")+
  geom_point(data = PCA.zoops$coord,
             aes(x = Dim.1, y = Dim.2),
             color = "black")+
  labs(color = "Filtered Particles", fill = "Filtered Particles",  shape = "Filtered Particles")+
  xlab("PC1") + ylab("PC2") +
