...
```{r export_filename}
export.file.name <- "OMSM_posteriors_PAP.RData"
```
...
if(TRUE){
  ...
  Data.sources$Source <- factor(NA, levels = Sources)
  Data.sources$Source[
  which(Data.sources$Depth < 50 & 
          Data.sources$d15NPhe < 0)
          # Data.sources$Size != "51-335 μm" & Data.sources$Size != ">335 μm")
  ] <- "Surface"
  Data.sources$Source[
    which(Data.sources$Depth > 200 & 
...
Data.zoops <- 
  subset(Data.zoops,
         (((Depth < 50 | Depth > 200)&
            (Event == "Night" & Epoch == "Epoch 3") &
             d15NPhe > -4 &
           # Sizecat == "Mixed" & # we're only going to look at whole community samples at PAP since individual taxa had such low Lys values
           (Depth > 100 | Size != ">5 mm"))) | # we're also ignoring large surface zooplankton which we think we're advected from outside the study site 
           (Sizecat != "Mixed" & Event == "Day")
           # d15NLys > min(Data.sources$d15NLys) # excluding a couple of zooplankton with anomolously low d15N Lys values
  )
...
```{r posterior_f_barplots, eval=TRUE, dpi=300, fig.asp=0.4, fig.width=8}
#### 0.2-0.5 and 1-2 mm zooplankton ####
plot.Surface <-
  ggplot()+
  geom_colh(data = subset(posts.long$f$mean, Size == "1-2 mm" | Size == "0.2-0.5 mm" | Size == ">5 mm"),
             aes(x = Value, y = Depth, group = Param, fill = Param),
            color = "grey30",
            position = position_stackv(reverse = TRUE),
            width = c(90,90,90,240,50,90,90,90,240,50,90,90,90,240,
                      90,90,90,240,50,90,90,90,240,50,90,90,90,240,
                      90,90,90,240,50,90,90,90,240,50,90,90,90,240)
            )+
  scale_y_reverse(breaks = c(0,100,200,300,400,500,750,1000)) + 
  scale_x_continuous(breaks = c(0,0.50,1))+
  labs(color="Source",fill="Source")+
  xlab("Source Contribution")+
  coord_cartesian(xlim = c(0,1))+
  facet_wrap(~ Size)
plot.Surface
...
noquote("Posterior mode for f(large)")
noquote(">5 mm zoops below 200 m")
min(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Large)
mean(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Large)
max(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Large)
noquote("1-2 mm zoops below 200 m")
min(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Large)
mean(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Large)
max(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Large)
noquote("0.2-0.5 mm zoops below 200 m")
min(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Large)
mean(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Large)
max(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Large)
...
noquote(">5 mm zoops below 200 m")
min(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Surface)
mean(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Surface)
max(subset(posts$f$mode, Size == ">5 mm" & Depth > 200)$Surface)
noquote("1-2 mm zoops below 200 m")
min(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Surface)
mean(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Surface)
max(subset(posts$f$mode, Size == "1-2 mm" & Depth > 200)$Surface)
noquote("0.2-0.5 mm zoops below 200 m")
min(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Surface)
mean(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Surface)
max(subset(posts$f$mode, Size == "0.2-0.5 mm" & Depth > 200)$Surface)

model <- lm(
  FWL ~ Depth + Size,
  data = subset(posts$trophic$mode, Size == ">5 mm" | Size == "0.2-0.5 mm" | Size == "1-2 mm")
)
anova(model)
plot(allEffects(model))
...
model <- lm(
  FWL ~ Depth,
  data = subset(posts$trophic$mode, Size == ">5 mm")
)
anova(model)
plot(allEffects(model))
...
model <- lm(
  FWL ~ Depth,
  data = subset(posts$trophic$mode, Size == "0.2-0.5 mm" | Size == "1-2 mm" )
)
anova(model)
plot(allEffects(model))
...
noquote("large EZ")
min(subset(posts$trophic$mode, Depth < 100 & Size == ">5 mm")$FWL)
mean(subset(posts$trophic$mode, Depth < 100 & Size == ">5 mm")$FWL)
max(subset(posts$trophic$mode, Depth < 100 & Size == ">5 mm")$FWL)
noquote("large MPZ")
min(subset(posts$trophic$mode, Depth > 200 & Size == ">5 mm")$FWL)
mean(subset(posts$trophic$mode, Depth > 200 & Size == ">5 mm")$FWL)
max(subset(posts$trophic$mode, Depth > 200 & Size == ">5 mm")$FWL)
noquote("all small")
min(subset(posts$trophic$mode, Size == "0.2-0.5 mm" | Size == "1-2 mm")$FWL)
mean(subset(posts$trophic$mode, Size == "0.2-0.5 mm" | Size == "1-2 mm")$FWL)
max(subset(posts$trophic$mode, Size == "0.2-0.5 mm" | Size == "1-2 mm")$FWL)
...
noquote("all zoops")
min(subset(posts$trophic$mode, Size == ">5 mm" | Size == "0.2-0.5 mm" | Size == "1-2 mm")$PTS)
mean(subset(posts$trophic$mode, Size == ">5 mm" | Size == "0.2-0.5 mm" | Size == "1-2 mm")$PTS)
max(subset(posts$trophic$mode, Size == ">5 mm" | Size == "0.2-0.5 mm" | Size == "1-2 mm")$PTS)

t.test(subset(posts$trophic$mode)$PTS,c(0,0,0))
```
