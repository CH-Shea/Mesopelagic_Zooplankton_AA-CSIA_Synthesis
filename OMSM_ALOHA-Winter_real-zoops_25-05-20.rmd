...
```{r posterior_f_barplots, eval=TRUE, dpi=300, fig.asp=0.6, fig.width=6}
#### 0.2-0.5 and 1-2 mm zooplankton ####
plot.Surface <-
  ggplot()+
  geom_colh(data = subset(posts.long$f$mean, Size == "1-2 mm" | Size == "0.2-0.5 mm"),
             aes(x = Value, y = Depth, group = Param, fill = Param),
            color = "grey30",
            position = position_stackv(reverse = TRUE),
            width = c(100,200,290,490)
            )+
  scale_y_reverse(breaks = c(0,100,200,300,400,500,700,1000,1500)) + 
  scale_x_continuous(breaks = c(0,0.50,1))+
