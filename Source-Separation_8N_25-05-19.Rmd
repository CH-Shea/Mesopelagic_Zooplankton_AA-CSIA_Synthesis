---
title: "Organic Matter Source Separation and Sample Selection - 8 N"
author: "Connor Shea"
...
tracers_select <- c("d15NLys","d15NGly") #,"d15NThr")
SDtracers_select <- c("Sd15NLys","SDd15NGly") #,"SDd15NThr")
...
Data.all <- read_excel("Data/AA-CSIA_8N.xlsx",
...
One of the first things we notice is that we are missing $\dN$ values for both Val and Thr in large particles below 500 m. As a result let's exclude these two amino acids for this site. We also notice that Phe is highly variable at 8 N and missing from the deepest large particle sample, suggesting it may not be very useful at this site. Unfortunately, we'll exclude Phe as well. 
...
```
 ...
 Otherwise, we see minimal depth dependence in most tracers at and below about 300 m.
...
Now we will do a series of statistical tests on our data. We will look at three specific tracer sets:
  ...
  3. A select group of $\dNAA$ values (Phe, Gly)
...
Most amino acid $\dN$ values are able to distinguish small and surface particles while only Gly statistically distinguishes large from surface particles. None, unfortunately, distinguish small and large particles, suggesting we won't be able to differentiate those to groups in a mixing model.
...
### Gly vs Lys plot {-}
Since we only have 2 tracers in our select group we will just do a simple plot of $\dN$ Ser vs Lys
