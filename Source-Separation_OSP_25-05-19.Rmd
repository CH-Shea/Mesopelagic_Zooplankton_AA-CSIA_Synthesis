...
Data.all <- read_excel("Data/AA-CSIA_OSP.xlsx",
                       sheet = "Data")[c(variables,tracers,SDtracers)]
...
ggplot()+
  ...
  labs(shape="Zooplankton",
       color="Particles")+
  ylab(expression(delta^{15}*N[" Pro"]))+
  xlab(expression(delta^{15}*N[" Phe"]))
...
ggplot()+
  ...
  geom_point(data = subset(Data.zoops, 
                           (Depth < 100 & Event == "Day") |
                           (Sizecat == "Metridia" | Sizecat == "Neocalanus")),
...
Here we can see a subset of surface particles have higher $\dN$ values in Lys any zooplankton that may conceivable feed on them. Given that TDF(Lys) = 1.1‰, lets filter out surface particles with δ15N(Lys) < 1‰. We will also forget about submicron particles.
...
ggplot()+
  geom_point(data = subset(Data.sources, 
                           Group == "Surface"),
             aes(x=d15NPhe, y=((d15NGlx-d15NPhe - 3.2)/6.2+1), color=Size),
             size = 3)+
  geom_point(data = subset(Data.zoops, 
                           (Depth <  100 & Event == "Day") |
                           (Sizecat == "Metridia" | Sizecat == "Neocalanus")),
